...
import { publishPool } from '../../../nftpool/helper';
        ...
        <div className={styles.card}>
          <div className={styles.header}>
            <div>
              ...
              <span>Stake {data.collectionName}</span>
            ...
            <ValidImage link={data.collectionLogo} alt="collection Logo" />
          ...
          <div className={styles.dets}>
            <div>
              <span>Rewards In Pool:</span>
              <p>{numberWithCommas(data.supply)} {data.rewardTokenSymbol}</p>
            </div>

            <div>
              <span>Total Staked:</span>
              <p>{numberWithCommas(data.amountStaked)} {data.collectionName}</p>
            </div>
            <div>
              <span>Reward Per NFT Per Day:</span>
              <p>{numberWithCommas(data.amountOfTokenPerDayPerNFT)} {data.rewardTokenSymbol}</p>
            </div>
            <div >
              <span id="impulsePass">Impulse Pass Slots:</span>
              <p id="impulsePass" style={{ color: "#F1D12B" }}> {numberWithCommas(data.impulseStaked)} / {numberWithCommas(data.slots)}</p>
            </div>
            {data.published ? <div>
              <span>Pool Link:</span>
              <p onClick={() => navigate("/earn/nft/" + data.poolAddress)}>Visit Pool</p>
            </div> : <></>}
            {data.supplied ? <div>
              <span>Published:</span>
